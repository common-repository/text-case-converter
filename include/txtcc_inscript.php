	...
	if(!function_exists('txtcc_admin_scripts')){
		function txtcc_admin_scripts() {
			...
			if(isset($_GET['page']) && !empty($_GET['page']) && sanitize_text_field($_GET['page']) == 'txtcc'){
				wp_enqueue_style( TXTCC_NAME.'-btsp',TXTCC_LIBS . 'libs/bootstrap-5.3.2/css/bootstrap.min.css',array(),TXTCC_VERSON);
				wp_enqueue_style( TXTCC_NAME.'-tooltip', TXTCC_LIBS . 'libs/zebra/zebra_tooltips.min.css',array(),TXTCC_VERSON); 
				wp_enqueue_style( TXTCC_NAME.'-owl-carousel-css', TXTCC_LIBS . 'libs/owl-carousel/css/owl.carousel.min.css',array(),TXTCC_VERSON); 
				wp_enqueue_style( TXTCC_NAME.'-owl-carousel-theme', TXTCC_LIBS . 'libs/owl-carousel/css/owl.theme.default.min.css',array(),TXTCC_VERSON);

				wp_enqueue_script( TXTCC_NAME.'-bootstrap_js',TXTCC_LIBS . 'libs/bootstrap-5.3.2/js/bootstrap.min.js', array( 'jquery' ), TXTCC_VERSON,true );
				wp_enqueue_script( TXTCC_NAME.'-tooltip_js', TXTCC_LIBS . 'libs/zebra/zebra_tooltips.min.js', array( 'jquery' ), TXTCC_VERSON,true);
				
				wp_enqueue_script( TXTCC_NAME.'-owl-carousel-js', TXTCC_LIBS . 'libs/owl-carousel/js/owl.carousel.min.js', array(), rand(),true);
				wp_enqueue_script( TXTCC_NAME.'-front_js', TXTCC_JS . 'txtfront.js', array( 'jquery' ), rand(),true);
				wp_enqueue_style( TXTCC_NAME.'-chnge-edit-admin', TXTCC_CSS . 'chnge-edit-admin.css',array(),rand());
			}
			...
			wp_enqueue_script( TXTCC_NAME.'-mce_js', TXTCC_JS . 'txtcc-mce.js', array( 'jquery' ), rand(),true);
