jQuery(function() {
	if (jQuery('#banners').length > 0) {
		jQuery('#banners').owlCarousel({
			...
			dots: false,
			autoplay: true,
			autoplaySpeed: 2000, // Corrected from autoPlaySpeed
	...
	if (jQuery('#kinsta_banners').length > 0) {
		...
		init : function(editor, url) {
			...
			editor.addButton('underline', {
				...
				onclick: function(){
					...
					if( underline != ''){
						Swal.fire({
							title:'Choose underline color and style',
							html:'<input type="text" class="txt_color_picker"> <select class="txt_border_style"><option value="solid">Solid</option><option value="dotted">Dotted</option><option value="dashed">Dashed</option><option value="double">Double</option><option value="wavy">Wavy</option> </select> ',
							showCancelButton: true,
							...
							if (result.value) {
								...
								var txt_border_style =  jQuery('.txt_border_style').val();
								
								tinymce.activeEditor.selection.setContent(tinymce.activeEditor.dom.createHTML('u', {style:'text-decoration-color:'+x+';text-decoration-style:'+txt_border_style},underline));
								editor.save();
								editor.isNotDirty = true;
